			...
			.ajax({
				...
				success : function(jtjlks) {
					for (var i = 0; i <= jtjlks.length - 1; i++) {
						...
						strHtml += "<td class='alignCenter'><input name='button'";
						strHtml += "		type='button' onclick=\"$('#name').html('"
								+ jtjlks[i].jtjlkName
								+ "');$('#divControll').show();$('#divControllBatch').hide();$('#Id').val('"
								+ jtjlks[i].jtjlkId
								+ "');$('#UserId').val('"
								+ jtjlks[i].jtjlkUserid
								+ "');document.body.scrollTop = document.body.scrollHeight;\" class='inputButton' value='操作' />";
						strHtml += "</td>";
...
function getOptionsFromForm(pi) {
	var opt = {
