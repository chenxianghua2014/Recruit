...
public class SendMessage
{
	...
	public static void main(String[] args)
	{
		...
		// //发送调用
		// xml=t.SendMessage("1111", "qqqq", "mima", "15201131591", "测试剩余与使用1",
		// "").toString();
		...
		// 非法关键词查询
		// xml=t.findIllegal("1111", "qqqq", "mima", "哦哦，哈哈！").toString();
		// System.out.println(xml);
		// xmlentity.setMessage("message");
		// xmlentity=t.readStringXmlCommen(xmlentity, xml);
		// System.out.println("非法关键词查询："+xmlentity.getMessage());
	...
	public StringBuffer SelSum(String userid, String account, String password)
	{
		try
		{
			url = new URL("http://sdk.kuai-xin.com:8888/sms.aspx?action=overage&userid=" + userid + "&account=" + account + "&password=" + password + "");
	...
	public StringBuffer SendMessage(String userid, String account, String password, String mobile, String content, String sendTime)
	{
		...
		try
		{
			String address = PropertiesUtils.getValue("address");
			userid = PropertiesUtils.getValue("userid");
			account = PropertiesUtils.getValue("account");
			password = PropertiesUtils.getValue("password");
			// 设置发送内容的编码方式
			String send_content = URLEncoder.encode(content.replaceAll("<br/>", " "), "UTF-8");// 发送内容

			url = new URL(address + "?action=send&userid=" + userid + "&account=" + account + "&password=" + password + "&mobile=" + mobile + "&content=" + send_content + "&sendTime=" + sendTime);
			con = (HttpURLConnection) url.openConnection();
			br = new BufferedReader(new InputStreamReader(con.getInputStream(), "UTF-8"));
			// br=new BufferedReader(new InputStreamReader(url.openStream()));
	...
	// 非法关键词查询
	public StringBuffer findIllegal(String userid, String account, String password, String content)
	{
		try
		{
			url = new URL("http://sdk.kuai-xin.com:8888/sms.aspx?action=checkkeyword&userid=" + userid + "&account=" + account + "&password=" + password + "&content=" + content + "");
			con = (HttpURLConnection) url.openConnection();
	...
	public StringBuffer StatusReport(String userid, String account, String password)
	{
		try
		{
			url = new URL("http://sdk.kuai-xin.com:8888/statusApi.aspx?action=query&userid=" + userid + "&account=" + account + "&password=" + password + "");
			con = (HttpURLConnection) url.openConnection();
