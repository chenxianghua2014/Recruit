...
public class SendEmail
{
	...
	public static boolean sendHtmlMail(MailSenderInfo mailInfo)
	{
		...
		try
		{
			...
			Address[] tos = null;
			tos = new InternetAddress[mailInfo.getToAddress().split(",").length];
			for (int i = 0; i < mailInfo.getToAddress().split(",").length; i++)
			{
				System.out.println(mailInfo.getToAddress().split(",")[i]);
				tos[i] = new InternetAddress(mailInfo.getToAddress().split(",")[i]);
			}
