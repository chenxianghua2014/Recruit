...
public class MainController {
	...
	return new ModelAndView("Main/LoginFrmFirst", "",
		session.getAttribute("userLoginSession"));
    }

    @RequestMapping(value = "/LoginFrm", method = RequestMethod.GET)
    public ModelAndView LoginFrm(HttpSession session) {
	return new ModelAndView("Main/LoginFrm", "",
		session.getAttribute("userLoginSession"));
    }
    ...
    public ModelAndView Group(
	    @RequestParam(value = "id", required = false) String id) {
	logInfo("Group", id);
	Xcxx xcxx = xcxxService
		.selectByPrimaryKey("09E57BF4-128B-6D1F-D988-269C158134C4");
    ...
    public ModelAndView PositionDetails(
	    @RequestParam(value = "id", required = false) String id) {
    ...
    public ModelAndView SearchEnterprise(
	    @RequestParam(value = "id", required = false) String id) {
	...
	if (session.getAttribute("userLoginSession") != null) {
	    ...
	    q.setPageNum(1);
	    q.setUserId(((Userinfo) session.getAttribute("userLoginSession"))
		    .getUserId());
    ...
    public ModelAndView MessageDetails(
	    @RequestParam(value = "id", required = false) String id) {
    ...
    public ModelAndView ApplyPosition(HttpSession session,
	    @RequestParam(value = "id", required = false) String id, String page)
	...
	List<Resume_jyjl> Jyjl = resume_jyjlService
		.getlistResumeJyjlByResumeId(id);
	List<Resume_xnjl> Xnjl = resume_xnjlService
		.getlistResumeXnjlByResumeId(id);
	List<Resume_xnzw> Xnzw = resume_xnzwService
		.getlistResumeXnzwByResumeId(id);
	List<Resume_sjjl> Sjjl = resume_sjjlService
		.getlistResumeSjjlByResumeId(id);
	List<Resume_sx> Sxjl = resume_sxService.getlistResumeSxByResumeId(id);
	List<Resume_gzjl> Gzjl = resume_gzjlService
		.getlistResumeGzjlByResumeId(id);
	List<Resume_xmjy> Xmjy = resume_xmjyService
		.getlistResumeXmjyByResumeId(id);
	List<Resume_pxjl> Pxjl = resume_pxjlService
		.getlistResumePxjlByResumeId(id);
	List<Resume_yynl> Yynl = resume_yynlService
		.getlistResumeYynlByResumeId(id);
	List<Resume_qtxx> Qtxx = resume_qtxxService
		.getlistResumeQtxxByResumeId(id);
	List<Resume_fj> Fj = resume_fjService.getlistResumeFjByResumeId(id);
	List<Resume_ITjn> ITjn = resume_ITjnService
		.getlistResumeITjnByResumeId(id);
	...
	mv.addObject("Zs", Zs);
	/*
	 * int isExist = jtjlkService.selectIsExist(params).size(); if (isExist
	 * > 0) {
	 * 
	 * // return new ModelAndView("Main/ShowAlert", "uri", page +
	 * "IsExist"); return new ModelAndView("Grjl/Yljl", "mv", mv); }
	 */

	/*
	 * Map<String, String> paramsMore = new HashMap<String, String>();
	 * paramsMore.put("userId", userinfo.getUserId()); // 不允许同时投递简历数量超过5家企业
	 * if (jtjlkService.selectIsExist(paramsMore).size() >= 5) { return new
	 * ModelAndView("Main/ShowAlert", "uri", page + "More"); }
	 */
	...
	List<Resume_jyjl> resume_Jyjls = resume_jyjlService
		.getlistResumeJyjlByResumeId(resume.getResumeId());
	Resume_jyjl resume_Jyjl = new Resume_jyjl();

	List<Resume_gzjl> resume_Gzjls = resume_gzjlService
		.getlistResumeGzjlByResumeId(resume.getResumeId());
	...
	if (resume.getResumeNypgw2().equals("")
		|| resume.getResumeNypgw2().equals(null)) {
	...
	jtjlk.setZzjgId("AD782CAC-E5E4-33FD-1763-67A6F967328C"); // 组织机构代码
								 // 设置为培训中心组织代码

	// jtjlk.setJtjlkLy("应聘者");//改为学位
	jtjlk.setJtjlkLy(resume_Jyjl.getResumeZyms());
	jtjlk.setJtjlkHkd(resume.getResumeRxqhkszcsProvince() + ","
		+ resume.getResumeRxqhkszcsCity()); // 户口所在地
	...
	List<Jtjlk> seList = jtjlkService.selectStatusByUserId(userinfo
		.getUserId());
	if (seList.size() == 0)
	...
	List<Jtjlk> jtjlkList = jtjlkService.selectBySfzh(userinfo
		.getUserSfzh());
	if (jtjlkList.size() != 0) {
	...
	if (userinfo == null) {
	    ...
	    List<MyApplication> myApplications = jtjlkService
		    .selectMyApplication(userinfo.getUserId());
	    return new ModelAndView("Main/MyApplication", "Applications",
		    myApplications);
	}
    }

    @RequestMapping(value = "/Revoke", method = RequestMethod.GET)
    public ModelAndView Revoke(HttpSession session,
	    @RequestParam(value = "id", required = false) String id) {
	logInfo("Revoke", id);
	Userinfo userinfo = (Userinfo) session.getAttribute("userLoginSession");
	if (userinfo == null) {
	    ...
	    return new ModelAndView("Main/ShowAlert", "uri",
		    "MyApplicationRevoke");
	}
    }
    ...
    public ModelAndView PrintTestCard(
	    @RequestParam(value = "id", required = false) String id) {
    ...
    public ModelAndView InstituteOf(
	    @RequestParam(value = "id", required = false) String id) {
	logInfo("InstituteOf", "");
	ModelAndView mv = new ModelAndView();
	Map<String, String> map = new HashMap<String, String>();
    ...
    public ModelAndView RecruitmentDynamics(
	    @RequestParam(value = "id", required = false) String id) {
    ...
    public ModelAndView RecruitmentDynamicsDetails(
